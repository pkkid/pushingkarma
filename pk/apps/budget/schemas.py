...
class RecurringItemSchema(Schema):
    name: str = Field(..., description='Normalized name used to group recurring payees')
    cadence: str = Field(..., description='Detected cadence (monthly or yearly)')
    count: int = Field(..., description='Number of matching transactions')
    first_date: datetime.date = Field(..., description='First transaction date')
    last_amount: Decimal = Field(..., description='Last transaction amount')
