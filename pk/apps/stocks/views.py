...
class StocksViewSet(viewsets.ModelViewSet):
    ...
    def csv(self, request, *args, **kwargs):
        ...
        stocks, title = self._get_stocks(request)
        if not stocks:
        ...
        for datestr in oldest.history.keys():
            writer.writerow([datestr] + [self._get_value(datestr, s) for s in stocks])
            if datestr < yearsago:
                break
        return response

    def _get_stocks(self, request):
    ...
    def _get_value(self, datestr, stock):
