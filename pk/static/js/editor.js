...
pk.editor2 = {
  ...
  init_elements: function() {
    ...
    this.mesage = this.menu.find('.message');
  ...
  init_editor: function() {
    ...
    this.set_height(this.MIN_HEIGHT);
    return this.editor;
  },
  ...
  toggle_editor: function() {
    ...
    if (!this.editing()) {
      this.editor.show();
      this.editor.animatecss('fadeInUp', function() {
        self.set_height(self.editor.height());
        self.codemirror.refresh();
        self.codemirror.focus();
      });
    } else {
      this.editor.animatecss('fadeOutDown', function() {
        self.editor.hide();
  ...
  update: function() {
    ...
    // check the editor is open and modified
    if (!self.editing() || _.isEqual(data, this.history.updated)) {
      return null;  // nothing to update
    }
    // live update markdown to html
    self.history.updated = data;
    var xhr = $.ajax({url:this.opts.markdown_url, data:data, type:'POST', dataType:'json'});
